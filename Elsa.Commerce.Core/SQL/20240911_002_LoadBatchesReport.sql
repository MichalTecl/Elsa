IF OBJECT_ID('LoadBatchesReport', 'P') IS NOT NULL
	DROP PROCEDURE LoadBatchesReport;

GO

CREATE PROCEDURE LoadBatchesReport (
	@projectId INT,
    @pageSize INT,
    @pageNumber INT,
    @materialId INT = NULL,	
...
BEGIN
	...
	SELECT y.BatchId,
		   ...
		   y.BatchProductionDt,
		   y.BatchStepsDone,
		   ISNULL(numberOfComponents.cnt, 0) NumberOfComponents,
		   ISNULL(numberOfCompositions.cnt, 0) NumberOfCompositions,
		   0 as numberOfMaterialSteps, 		   		   
		   ISNULL(numberOfOrders.cnt, 0) NumberOfOrders,
		   y.BatchPrice,
		   y.InvoiceNumber,
		   ISNULL(numberOfStockEvents.cnt, 0) NumberOfStockEvents,
		   ISNULL(numberOfSaleEvents.cnt, 0) NumberOfSaleEvents,
		   ISNULL(segments.cnt, 1) NumberOfSegments,
