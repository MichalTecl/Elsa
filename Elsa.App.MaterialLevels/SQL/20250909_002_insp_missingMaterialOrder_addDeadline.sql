...
BEGIN
	...
	*/
	...
	AS
	(
	SELECT m.Id MaterialId, m.Name, moe.OrderDt, COALESCE(moe.DeliveryDeadline, DATEADD(day, COALESCE(m.OrderFulfillDays, sup.OrderFulfillDays, 30), moe.OrderDt)) fulfillLimit
	...
	SELECT N'Nedošlo k nasladnění suroviny' IssueType,
		   ...
		   cte.Name + N': bylo očekáváno naskladnění do ' + FORMAT(cte.FulfillLimit, 'dd.MM.yyyy') Message,
		   cte.MaterialId "data:MaterialId",
		   '/UI/Controls/MaterialLevels/InspectorActions/MaterialDeliveryExpectationPostpone.html' "ActionControlUrl_SetDeadline",
		   'SET_DEADLINE' "ActionName_SetDeadline"
