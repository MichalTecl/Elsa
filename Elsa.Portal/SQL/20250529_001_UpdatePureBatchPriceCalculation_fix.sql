...
CREATE PROCEDURE [dbo].[UpdatePureBatchPriceCalculation]
AS
BEGIN
    ...
    DECLARE @insCounterBef INT = 0;
	DECLARE @insCounterAft INT = 0;

	WHILE(1 = 1)
	BEGIN

	    SELECT @insCounterBef = COUNT(*) FROM  PureBatchPriceCalculation;
		...
		FROM (
			...
			SELECT @insCounterAft = COUNT(*) - @insCounterBef FROM  PureBatchPriceCalculation;

			PRINT 'ROWCOUNT: ' + CAST(@insCounterAft AS VARCHAR)
