...
ALTER PROCEDURE [dbo].[CalculateBatchUsages] (@ProjectId INT, @BatchId INT = NULL, @MaterialId INT = NULL, @repDate DATETIME = NULL, @debug BIT = NULL, @inventoryId INT = NULL)
AS
BEGIN	
	...
	SELECT mbc.ComponentId, @T_SUBTRACT, dbo.ConvertToUnit(@ProjectId, mbc.Volume, mbc.UnitId, src.CalcUnitId)
	  ...
	  INNER JOIN MaterialBatch composition ON (mbc.CompositionId = composition.Id)
	  WHERE composition.Created <= @repDate;
	...
	WHERE po.OrderStatusId NOT IN (/*6,*/ 7, 8) -- 6 = returned => it shouldn't be automatically returned back to stock
	  AND oimb.AssignmentDt <= @repDate;

	-- Sale events
	INSERT INTO @stat (BatchId, OperationType, Amount)
	SELECT sea.BatchId, @T_SUBTRACT, sea.AllocatedQuantity - ISNULL(sea.ReturnedQuantity, 0)
	  FROM SaleEventAllocation sea
	  JOIN SaleEvent se ON (sea.SaleEventId = se.Id)
     WHERE se.EventDt <= @repDate
	   AND sea.BatchId IN (SELECT Id FROM @batches);
