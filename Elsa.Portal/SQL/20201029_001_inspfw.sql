...
CREATE PROCEDURE [dbo].[inspfw_addIssue] (@sessionId INT, @typeName NVARCHAR(200), @code NVARCHAR(200), @message NVARCHAR(1000))
AS
BEGIN
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_getActiveIssuesSummary')
	DROP PROCEDURE [inspfw_getActiveIssuesSummary];
GO


CREATE PROCEDURE [dbo].[inspfw_getActiveIssuesSummary] (@projectId INT, @userId INT = NULL)
AS
BEGIN
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_getInspectionProcedures')
	DROP PROCEDURE [inspfw_getInspectionProcedures];
GO


CREATE PROCEDURE [dbo].[inspfw_getInspectionProcedures] 
AS
BEGIN
	SELECT name 
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_getIssuesToMail')
	DROP PROCEDURE [inspfw_getIssuesToMail];
GO
CREATE PROCEDURE [dbo].[inspfw_getIssuesToMail] (@projectId INT)
AS
BEGIN
...
CREATE PROCEDURE [dbo].[inspfw_openSession] (@projectId INT)
AS
BEGIN
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_runInspection')
	DROP PROCEDURE [inspfw_runInspection];
GO

CREATE PROCEDURE [dbo].[inspfw_runInspection] (@projectId INT, @sessionId INT, @retryIssueId INT = null, @spName VARCHAR(300) = null)
AS
BEGIN
	IF (@spName IS NULL)
	BEGIN
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_setIssueAction')
	DROP PROCEDURE [inspfw_setIssueAction];
GO


CREATE PROCEDURE [dbo].[inspfw_setIssueAction] (@issueId INT, @controlUrl NVARCHAR(300), @actionName NVARCHAR(300))
AS
BEGIN	
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_setIssueDataInt')
	DROP PROCEDURE [inspfw_setIssueDataInt];
GO

CREATE PROCEDURE [dbo].[inspfw_setIssueDataInt] (@issueId INT, @property NVARCHAR(100), @value INT, @isArray BIT = 0)
AS
BEGIN	
...
IF EXISTS(SELECT TOP 1 1 FROM sys.procedures WHERE name = 'inspfw_setIssueDataString')
	DROP PROCEDURE [inspfw_setIssueDataString];
GO


CREATE PROCEDURE [dbo].[inspfw_setIssueDataString] (@issueId INT, @property NVARCHAR(100), @value NVARCHAR(1000), @isArray BIT = 0)
AS
BEGIN									
