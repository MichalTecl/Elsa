...
BEGIN
    ...
    DECLARE @pk_column_id INT;
    DECLARE @actual_pk_sql_type NVARCHAR(128);
    DECLARE @actual_identity BIT;
    DECLARE @pk_constraint_name SYSNAME;
    DECLARE @pk_matches BIT = 0;
    ...
    SELECT
        @pk_column_id = c.column_id,
        @actual_identity = CASE WHEN COLUMNPROPERTY(c.object_id, c.name, 'IsIdentity') = 1 THEN 1 ELSE 0 END,
        @actual_pk_sql_type = LOWER(
            CASE
    ...
    WHERE c.object_id = @table_object_id
      AND c.name = @pk_column_name;

    IF @pk_column_id IS NULL
    BEGIN
        RAISERROR(N'robormsp_ensuretable expected an existing primary key column', 16, 1);
        RETURN;
    END;

    IF ISNULL(@actual_identity, 0) <> CASE WHEN @pk_identity = 1 THEN 1 ELSE 0 END
    BEGIN
        RAISERROR(N'robormsp_ensuretable does not support changing IDENTITY on primary keys', 16, 1);
        RETURN;
    END;
    ...
    IF EXISTS(
        ...
        HAVING COUNT(*) > 1)
    BEGIN
        RAISERROR(N'robormsp_ensuretable does not support composite primary keys', 16, 1);
        RETURN;
    END;

    SELECT TOP 1
        @pk_constraint_name = kc.name
    FROM sys.key_constraints kc
    WHERE kc.parent_object_id = @table_object_id
      AND kc.type = 'PK';
    ...
    IF @pk_constraint_name IS NOT NULL
        AND EXISTS(
            ...
            JOIN sys.index_columns ic
               ...
               AND ic.index_id = kc.unique_index_id
            WHERE kc.parent_object_id = @table_object_id
              AND kc.name = @pk_constraint_name
            GROUP BY kc.name
            HAVING COUNT(*) = 1
               AND MAX(CASE WHEN ic.column_id = @pk_column_id THEN 1 ELSE 0 END) = 1)
    BEGIN
        SET @pk_matches = 1;
    END;

    IF @actual_pk_sql_type = @normalized_pk_sql_type
       AND @pk_matches = 1
    BEGIN
        RAISERROR(N'robormsp_ensuretable: table %s already matches requested PK definition', 10, 1, @table_name) WITH NOWAIT;
        RETURN;
    END;
    ...
    WHERE EXISTS(
        ...
        WHERE fkc.constraint_object_id = fk.object_id
          ...
          AND fkc.referenced_column_id = @pk_column_id);
    ...
    IF @pk_constraint_name IS NOT NULL
    BEGIN
        RAISERROR(N'robormsp_ensuretable: dropping PK constraint %s on table %s', 10, 1, @pk_constraint_name, @table_name) WITH NOWAIT;
        SET @sql = N'ALTER TABLE ' + QUOTENAME(@table_schema_name) + N'.' + QUOTENAME(@table_name) +
            N' DROP CONSTRAINT ' + QUOTENAME(@pk_constraint_name) + N';';
        EXEC(@sql);
    END;

    IF @actual_pk_sql_type <> @normalized_pk_sql_type
    BEGIN
        RAISERROR(N'robormsp_ensuretable: altering PK column %s.%s from %s to %s', 10, 1, @table_name, @pk_column_name, @actual_pk_sql_type, @normalized_pk_sql_type) WITH NOWAIT;
        SET @sql = N'ALTER TABLE ' + QUOTENAME(@table_schema_name) + N'.' + QUOTENAME(@table_name) +
            N' ALTER COLUMN ' + QUOTENAME(@pk_column_name) + N' ' + @normalized_pk_sql_type + N' NOT NULL;';
        EXEC(@sql);
    END;

    RAISERROR(N'robormsp_ensuretable: recreating PK constraint on %s', 10, 1, @table_name) WITH NOWAIT;
    SET @sql = N'ALTER TABLE ' + QUOTENAME(@table_schema_name) + N'.' + QUOTENAME(@table_name) +
        N' ADD CONSTRAINT ' + QUOTENAME(N'PK_' + @table_name) +
        N' PRIMARY KEY CLUSTERED (' + QUOTENAME(@pk_column_name) + N');';
    EXEC(@sql);
    ...
    RAISERROR(N'robormsp_ensuretable: table %s updated, dropped FK=%d, recreated FK=%d', 10, 1, @table_name, @dropped_fk_count, @recreated_fk_count) WITH NOWAIT;
END
GO
