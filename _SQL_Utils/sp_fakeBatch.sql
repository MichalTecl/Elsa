ALTER PROCEDURE [dbo].[sp_fakeBatch] @materialId INT, @qty DECIMAL, @authorId INT
AS
BEGIN
	...
	UPDATE MaterialBatch -- 59
	   SET BatchNumber = 'FAKE_' + RIGHT((SELECT TOP 1 Name FROM Material WHERE Id = @materialId) + '_' + CAST(@newBatchId AS VARCHAR), 59)
	 WHERE Id = @newBatchId;
