...
BEGIN
	...
	SELECT @price = ISNULL(Price, 0) + ISNULL(ProductionWorkPrice, 0) + ISNULL(fbc.Value, 0), 
...
ELSE
BEGIN
	DELETE FROM BatchPriceComponent;
END
...
IF EXISTS(SELECT * FROM sys.procedures WHERE Name = 'GetFixedCostReport')
BEGIN
	DROP PROCEDURE GetFixedCostsReport;
END

GO

CREATE PROCEDURE GetFixedCostReport 
(
	@projectId INT
)
AS
BEGIN
	...
	SELECT RIGHT('0' + TRIM(STR(fcv.Month)), 2) + '/' + TRIM(STR(fcv.Year)), fct.Name, fcv.Value,  
	fct.PercentToDistributeAmongProducts 
		FROM FixedCostType fct
		JOIN FixedCostValue fcv ON (fct.Id = fcv.FixedCostTypeId)
		ORDER BY fcv.Year, fcv.Month, fct.Name
END
