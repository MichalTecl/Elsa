...
CREATE PROCEDURE ApplyFixedCosts (
	@projectId INT,
	@userId INT,
	@year INT,
	@month INT,
...
BEGIN
	...
	BEGIN TRY
		...
		DECLARE @toDistribute DECIMAL(19, 5) = (
			SELECT fcv.Value / 100 * CAST(fct.PercentToDistributeAmongProducts AS DECIMAL(19, 5))
		...
		DECLARE @totalDistrib DECIMAL(19, 5) = (
		...
		VALUES (@year, @month, @toDistribute, @totalDistrib, @toDistribute / @totalDistrib, @projectId, @userId, GETDATE());
