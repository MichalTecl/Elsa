IF EXISTS(SELECT TOP 1 1 FROm sys.procedures WHERE name = 'CalculateBatchUsages')
BEGIN
	DROP PROCEDURE CalculateBatchUsages;
END
...
BEGIN	
	...
	-- Production steps
	INSERT INTO @stat (BatchId, OperationType, Amount)
	SELECT x.batchId, @T_LIMIT, MIN(x.Amount)	
	FROM (
		SELECT b.Id batchId, requiredStep.Id stepId, SUM(dbo.ConvertToUnit(@projectId, ISNULL(batchStep.ProducedAmount,0), m.NominalUnitId, src.CalcUnitId)) as Amount 
		  FROM MaterialBatch b 
		  INNER JOIN @batches src ON (b.Id = src.Id)
		  INNER JOIN Material m ON (b.MaterialId = m.Id)
		  INNER JOIN MaterialProductionStep requiredStep ON (requiredStep.MaterialId = b.MaterialId)
		  LEFT JOIN BatchProductionStep     batchStep    ON (batchStep.BatchId = b.Id AND batchStep.StepId = requiredStep.Id)
		GROUP BY b.Id,  requiredStep.Id, m.NominalUnitId) as x
    GROUP BY x.batchId;
	...
	-- Used in production steps
	INSERT INTO @stat (BatchId, OperationType, Amount)
	SELECT bsb.SourceBatchId, @T_SUBTRACT, dbo.ConvertToUnit(@ProjectId, bsb.UsedAmount, bsb.UnitId, src.CalcUnitId)
	  FROM BatchProuctionStepSourceBatch bsb
	  INNER JOIN @batches src ON (src.Id = bsb.SourceBatchId);
	...
	WHERE po.OrderStatusId NOT IN (6, 7, 8); 
