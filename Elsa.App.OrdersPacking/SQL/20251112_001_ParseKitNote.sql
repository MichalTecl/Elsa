CREATE OR ALTER PROCEDURE [dbo].[ParseKitNote](@projectId INT, @orderId INT = NULL)
AS
BEGIN
    DECLARE @cuts TABLE (
        Id INT IDENTITY(1,1), 
	...
	BEGIN
		...
		BEGIN TRY
			...
			BEGIN
				...
				DECLARE @pos INT = ISNULL(PATINDEX('%Do _. sady%', @inp), 0);
				IF (@pos = 0)
					BREAK;
				...
				DECLARE @kitNameEndPos INT = ISNULL(PATINDEX('%si přeji:#%', @inp), 0);
				PRINT @kitNameEndPos;
				...
				DECLARE @nextPos INT = ISNULL(PATINDEX('%Do _. sady%', @inp), 0);
				IF (@nextPos = 0)
		...
		END TRY
		BEGIN CATCH
			INSERT INTO @cuts (OrderId) VALUES (@orderId);
		END CATCH
	END
